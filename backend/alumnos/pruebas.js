...
router.get('/insertar', async (req, res) => {
  ...
  try {
    ...
    const [clase] = await poolPromise.query(
      `INSERT INTO Clases (nombre, hora_inicio, hora_fin, capacidad_maxima, nivel, id_maestro)
       VALUES (?, ?, ?, ?, ?, ?)`,
      ['Karate Test', '18:00', '19:00', 10, 'Básico', maestro.insertId]
    );
    ...
    for (let i = 0; i < 3; i++) {
      ...
      await poolPromise.query(
        ...
        [
          alumno.insertId,
          membresia.insertId,
          estados[i] === 'Con deuda' ? 'pendiente' : 'pagado'
        ]
      );
      ...
      alumnos.push({
        ...
        horarios: [`${clase.hora_inicio} - ${clase.hora_fin}`],
        estado: estados[i],
    ...
    res.json({
      message: 'Alumnos de prueba insertados',
      alumnos: ultimoRegistro.alumnos
    });
...
router.get('/borrar', async (req, res) => {
  ...
  try {
    ...
    console.error(err);
    res.status(500).json({
      message: 'Error reiniciando tablas',
      error: err.message
    });
  }
});
