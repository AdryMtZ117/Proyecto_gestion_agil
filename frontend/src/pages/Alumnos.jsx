...
function Alumnos() {
  ...
  const [formData, setFormData] = useState({
    ...
    estado: 'activo', id_membresia: '', id_clase: '', foto: null
  });
  ...
  const fetchClases = async () => {
    try {
      const res = await axios.get('http://localhost:3000/api/alumnos1/clases');
      setClasesList(res.data);
    } catch (err) { console.error(err); }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    data.append('id_clase', formData.id_clase);
  ...
  const alumnosFiltrados = alumnos.filter(a => {
    ...
    return (
      ...
      (filtroEst ? estado === filtroEst : true)
    );
  });
        ...
        <div className="modal-overlay">
          <div className="modal-content">
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="form-group">
                <label>Asignar a Clase (Opcional)</label>
